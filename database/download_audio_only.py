...
#NS added variables for paths, edited respective parser.add_argument() calls below
FFMPEG_PATH = "/bin/anaconda3/bin/ffmpeg"
FFPROBE_PATH = "/bin/anaconda3/bin/ffprobe"
...
def download_yt_video(ytid, ts_start, ts_end, output_dir, ffmpeg_path, ffprobe_path,
    ...
    audio_filepath = os.path.join(output_dir, media_filename + '.' + audio_format)
...
def download_subset_file(subset_url, subset_name, dataset_dir):
    """
...
def download_subset_videos(subset_path, subset_name, data_dir, ffmpeg_path, ffprobe_path,
    ...
    with open(subset_path, 'r') as f:
        subset_data = csv.reader(f, delimiter='\t')
        ...
        try:
            for row_idx, row in enumerate(subset_data):
                ...
                media_filename = get_media_filename(ytid, ts_start, ts_end)
                audio_filepath = os.path.join(data_dir, media_filename + '.' + ffmpeg_cfg.get('audio_format', 'flac'))
...
def download_subset(subset_path, subset_name, dataset_dir, ffmpeg_path, ffprobe_path,
                    num_workers, **ffmpeg_cfg):
    """
    ...
    if is_url(subset_path):
        subset_path = download_subset_file(subset_path, subset_name, dataset_dir)

    data_dir = init_subset_data_dir(dataset_dir, subset_name)

    download_subset_videos(subset_path, subset_name, data_dir, ffmpeg_path, ffprobe_path,
                           num_workers, **ffmpeg_cfg)
...
def download_audioset(data_dir, ffmpeg_path, ffprobe_path, eval_segments_path,
    ...
    download_subset(eval_segments_path, "evaluation", data_dir, ffmpeg_path, ffprobe_path,
                    num_workers, **ffmpeg_cfg)
    download_subset(balanced_train_segments_path, "balanced_train", data_dir, ffmpeg_path, ffprobe_path,
                    num_workers, **ffmpeg_cfg)
    #download_subset(unbalanced_train_segments_path, "unbalanced_train", data_dir, ffmpeg_path, ffprobe_path,
    #                num_workers, **ffmpeg_cfg)
